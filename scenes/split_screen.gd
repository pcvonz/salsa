...
func _ready():
	...
	print(get_node("bottom/viewport").get_rect())
...
func _process(delta):
	...
	get_node("top").set_size(Vector2(win.x, win.y/2))
	get_node("top").set_global_pos(Vector2(0, 0))
	get_node("bottom").set_size(Vector2(win.x, win.y/2))
	get_node("bottom").set_global_pos(Vector2(0, win.y/2))
